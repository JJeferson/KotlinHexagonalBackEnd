...
class AuthInterceptor(TokenFixo: String?) : HandlerInterceptor {
    ...
    override fun preHandle(request: HttpServletRequest, response: HttpServletResponse, handler: Any): Boolean {
        ...
        try {
            ...
            val error = Error("401", "Unauthorized Request", "Token expirado", 1)
            response.writer.write(Gson().toJson(error))
            response.status = 401
            response.contentType = "application/json"
            return false
        } catch (e: JWTDecodeException) {
            val error = Error("401", "Unauthorized Request", "Token inválido", 1)
            response.writer.write(Gson().toJson(error))
            response.status = 401
            response.contentType = "application/json"
            return false
        } catch (e: Exception) {
            val error = Error("401", "Unauthorized Request", e.message, 1)
            response.writer.write(Gson().toJson(error))
            response.status = 401
            response.contentType = "application/json"
            return false
